...
#	print 'sed ' + cmd1 + cmd2 + 'lmp_GK_KDP.sh > ./lmp_GK_KDP_' + str(a + 1) + '.sh' #for debugging
			...
			## ## ## Execute sed command
			os.system('sed ' + cmd1 + cmd2 + 'lammps.script.temp > ./tmp.lammps.script.' + str(iseed) + '.' + str(itemp) + '.' + str(istrain) + '.sh')
			...
			os.system('qsub -p -1000 tmp.lammps.script.' + str(iseed) + '.' + str(itemp) + '.' + str(istrain) + '.sh')
## ## ## END FILE
